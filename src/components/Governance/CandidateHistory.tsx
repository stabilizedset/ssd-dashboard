...
async function formatProposals(epoch: number, proposals: any[]): Promise<Proposal[]> {
  const currentTotalStake = await getTokenTotalSupply(SSDS.addr);
  const initializeds = await Promise.all(proposals.map((p) => getIsInitialized(SSDS.addr, p.candidate)));
  const approves = await Promise.all(proposals.map((p) => getApproveFor(SSDS.addr, p.candidate)));
  const rejecteds = await Promise.all(proposals.map((p) => getRejectFor(SSDS.addr, p.candidate)));
  const supplyAts = await Promise.all(proposals.map(async (p) => {
    const at = (p.start + p.period - 1);
    if (epoch > at) {
      return await getTotalBondedAt(SSDS.addr, at);
    }
