...
import {SSDS} from "../../constants/tokens";
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      const [
        ...
        getRecordedVote(SSDS.addr, user, candidate),
      ...
      if (!isCancelled) {
        ...
        setUserStake(toTokenUnitsBN(userStakeStr, SSDS.decimals));
      }
    }
    updateUserInfo();
  ...
  useEffect(() => {
    ...
    async function updateUserInfo() {
      let [
        ...
        getApproveFor(SSDS.addr, candidate),
        getRejectFor(SSDS.addr, candidate),
        getTokenTotalSupply(SSDS.addr),
        getEpoch(SSDS.addr),
        getStartFor(SSDS.addr, candidate),
        getPeriodFor(SSDS.addr, candidate),
        getIsInitialized(SSDS.addr, candidate),
      ]);
      ...
      if (epochN > endsAfter) {
        totalStakeStr = await getTotalBondedAt(SSDS.addr, endsAfter);
      }

      if (!isCancelled) {
        setApproveFor(toTokenUnitsBN(approveForStr, SSDS.decimals));
        setRejectFor(toTokenUnitsBN(rejectForStr, SSDS.decimals));
        setTotalStake(toTokenUnitsBN(totalStakeStr, SSDS.decimals));
        setEpoch(epochN);
